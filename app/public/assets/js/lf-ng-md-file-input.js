(function (window, angular, undefined) {
    ...
    lfNgMdFileinput.run(['$templateCache', function ($templateCache) {
        $templateCache.put('lfNgMdFileinput.html', [
            ...
            '{{ lfFiles.length == 1 ? lfFiles[0].lfFileName : lfFiles.length+" dosya seçildi" }}',
            '</span>',
            '</div>',
    ...
    lfNgMdFileinput.directive('lfNgMdFileInput', ['$q', '$compile', '$timeout', function ($q, $compile, $timeout) {
        return {
            ...
            link: function (scope, element, attrs, ctrl) {
                ...
                scope.strCaptionPlaceholder = 'Dosya Seçiniz';

                scope.strCaptionDragAndDrop = 'Dosyayı Sürükle ve Bırak';

                scope.strCaptionBrowse = 'Dosya Seç';

                scope.strCaptionRemove = 'Temizle';

                scope.strCaptionSubmit = 'Tamam';
